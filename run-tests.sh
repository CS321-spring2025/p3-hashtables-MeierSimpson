...
do
    ...
    mv linear-dump.txt linear-dump-$load.txt
    mv double-dump.txt double-dump-$load.txt

    echo
    diff -w -B linear-dump-$load.txt test-cases/word-list-$load-linear-dump.txt > diff-linear-$load.out
    if test "$?" = 0
    then
    ...
    diff -w -B double-dump-$load.txt test-cases/word-list-$load-double-dump.txt > diff-double-$load.out
    if test "$?" = 0
    then
